...
async def show_focus_menu(message: Message):
    ...
    try:
        ...
        if session:
            ...
            keyboard = get_session_control_keyboard(is_paused=(session["status"] == "paused"))
        else:
            # Нет активной сессии
...
async def start_focus_session(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await callback.message.edit_text(
            text, reply_markup=get_session_control_keyboard(), parse_mode="HTML"
        )
...
async def pause_focus_session(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await callback.message.edit_text(
            text, reply_markup=get_session_control_keyboard(is_paused=True), parse_mode="HTML"
        )
...
async def stop_focus_session(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await callback.message.edit_text(
            text, reply_markup=get_focus_menu_keyboard(), parse_mode="HTML"
        )

        logger.info(
            f"Сессия {session['id']} остановлена ({'завершена' if is_completed else 'отменена'})"
        )
...
async def show_focus_settings(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.message.edit_text(
            text, reply_markup=get_focus_settings_keyboard(settings), parse_mode="HTML"
        )
...
async def toggle_auto_break(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.answer(f"Авто-перерыв {'включен ✅' if new_value else 'выключен ❌'}")
...
async def show_period_stats(callback: CallbackQuery):
    ...
    try:
        ...
        keyboard.row(InlineKeyboardButton(text="🔙 Назад", callback_data="focus_stats"))
...
async def show_focus_help(callback: CallbackQuery):
    ...
    keyboard = InlineKeyboardBuilder()
    keyboard.row(InlineKeyboardButton(text="🔙 Назад", callback_data="focus"))
