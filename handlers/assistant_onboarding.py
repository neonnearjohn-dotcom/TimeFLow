...
def load_questions() -> Dict[str, Any]:
    ...
    try:
        json_path = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "data", "onboarding_questions.json"
        )
            ...
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="🔄 Настроить заново", callback_data="onb_restart_confirmed"
                    )
                ],
...
def get_navigation_keyboard(
    question_index: int, total_questions: int, required: bool = True
    ...
    if question_index > 0:
        buttons.append(InlineKeyboardButton(text="◀️ Назад", callback_data="onb_previous_question"))

    # Кнопка "Пропустить" (если вопрос не обязательный)
    if not required:
        buttons.append(InlineKeyboardButton(text="➡️ Пропустить", callback_data="onb_skip_current"))

    if buttons:
        builder.row(*buttons)

    # Кнопка отмены
    builder.row(InlineKeyboardButton(text="❌ Отмена", callback_data="onb_cancel"))
...
async def show_next_question(obj: Any, state: FSMContext):
    ...
    if question_type == "select":
        ...
        await show_multiselect_question(
            obj, state, question, current_index, len(all_questions), text
        )

    elif question_type in ["text", "number", "date", "time", "list"]:
        await show_input_question(
            obj, state, question, question_type, current_index, len(all_questions), text, required
        )
...
async def show_select_question(obj, question, current_index, total_questions, text):
    ...
    for option in question["options"]:
        builder.row(
            InlineKeyboardButton(
                text=option["label"], callback_data=f"onb_answer_{question['id']}_{option['value']}"
            )
        )

    # Добавляем навигацию
    nav_keyboard = get_navigation_keyboard(
        current_index, total_questions, question.get("required", True)
    )
...
async def show_multiselect_question(obj, state, question, current_index, total_questions, text):
    ...
    if selected or not question.get("required", True):
        builder.row(
            InlineKeyboardButton(text="✅ Далее", callback_data=f"onb_multi_done_{question['id']}")
        )
...
async def show_input_question(
    obj, state, question, question_type, current_index, total_questions, text, required
):
    ...
    if question_type == "date" and question.get("validation") == "future_date":
        ...
        keyboard = get_number_quick_select_keyboard(
            question["min"], question["max"], unit=question.get("unit", "")
        )
    else:
        ...
        await callback.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        InlineKeyboardButton(
                            text="✅ Создать план", callback_data="onb_generate_plan_from_profile"
                        )
                    ],
                    [
                        InlineKeyboardButton(
                            text="🔄 Изменить настройки", callback_data="onb_restart_confirmed"
                        )
                    ],
...
async def generate_plan_from_existing_profile(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        "<b>⏳ Генерирую план на основе ваших настроек...</b>", parse_mode="HTML"
    )
    ...
    try:
        ...
        await state.update_data(generated_plan=plan.dict(), current_start_day=1)
...
@router.callback_query(
    F.data.startswith("onb_category_"), StateFilter(AssistantOnboardingStates.choosing_category)
)
...
async def process_select_answer(callback: CallbackQuery, state: FSMContext):
    ...
    parts = callback.data.split(
        "_", 3
    )  # Ограничиваем split для сохранения подчеркиваний в значении
    question_id = parts[2]
...
async def process_date_input(message: Message, state: FSMContext):
    ...
    try:
        ...
        await profile_db.save_onboarding_answer(
            message.from_user.id, question_id, date_value.isoformat()
        )
...
@router.callback_query(
    F.data == "onb_confirm_final", StateFilter(AssistantOnboardingStates.confirming_data)
)
async def finalize_onboarding(callback: CallbackQuery, state: FSMContext):
    ...
    success = await profile_db.finalize_onboarding(
        callback.from_user.id, category, answers, constraints
    )
    ...
    if success:
        ...
        try:
            # Генерируем план
            plan = await generate_plan(
                category=category, answers=answers, constraints=constraints, horizon_days=15
            )
...
@router.callback_query(
    F.data.startswith("plan:"), StateFilter(AssistantOnboardingStates.viewing_generated_plan)
)
            ...
            await callback.message.edit_text(
                ...
                reply_markup=InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            InlineKeyboardButton(
                                text="📊 Мой прогресс", callback_data="ai_show_plan"
                            )
                        ],
                        [
                            InlineKeyboardButton(
                                text="◀️ В меню ассистента", callback_data="assistant_menu"
                            )
                        ],
                    ]
                ),
...
def get_confirmation_keyboard() -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(text="✅ Все верно, продолжить", callback_data="onb_confirm_final")
    )
    builder.row(
        InlineKeyboardButton(text="✏️ Изменить ответы", callback_data="onb_restart_confirmed")
    )
    builder.row(InlineKeyboardButton(text="❌ Отмена", callback_data="onb_cancel"))
...
def get_number_quick_select_keyboard(
    ...
    for i in range(0, len(options), 3):
        ...
        builder.row(
            *[
                InlineKeyboardButton(text=f"{opt}{unit}", callback_data=f"onb_number_{opt}")
...
async def process_multiselect(callback: CallbackQuery, state: FSMContext):
    ...
    if callback.data.startswith("onb_multi_done_"):
        ...
        current_index = data.get("current_question_index", 0)
        await state.update_data(
            answers=answers, current_question_index=current_index + 1, multiselect_temp=[]
        )
...
async def show_current_plan(callback: CallbackQuery, state: FSMContext):
        ...
        await callback.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        InlineKeyboardButton(
                            text="🎯 Создать план", callback_data="ai_assistant_start"
                        )
                    ],
                    [InlineKeyboardButton(text="◀️ Назад", callback_data="assistant_menu")],
                ]
    ...
    await state.update_data(
        viewing_plan=profile.plan.dict(), current_start_day=1, is_view_mode=True
    )
    await state.set_state(PlanPreviewStates.viewing)
