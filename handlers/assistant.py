...
async def handle_assistant_menu(message: Message, state: FSMContext):
    ...
    await message.answer(
        welcome_text, reply_markup=get_assistant_menu_keyboard(), parse_mode="HTML"
    )
...
async def show_assistant_menu(callback: CallbackQuery):
    ...
    if has_plan:
        builder.row(
            InlineKeyboardButton(text="📋 Посмотреть мой план", callback_data="ai_show_plan")
        )
        # Добавляем отдельную кнопку для создания нового плана
        builder.row(
            InlineKeyboardButton(text="🔄 Создать новый план", callback_data="ai_create_new_plan")
        )
    else:
        # Если плана нет - только кнопка создания
        builder.row(
            InlineKeyboardButton(
                text="🎯 Создать персональный план", callback_data="ai_assistant_start"
            )
        )
    ...
    builder.row(
        ...
        InlineKeyboardButton(text="📊 Статистика", callback_data="assistant_stats"),
    )
...
async def process_scenario(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(ASSISTANT_MESSAGES["thinking"], parse_mode="HTML")
    ...
    try:
        ...
        if not openai_assistant.has_api_key():
            ...
            response = await openai_assistant.send_message(
                message="", context=history, scenario=scenario  # Для сценариев сообщение не нужно
            )
            ...
            if response["success"]:
                ...
                await assistant_db.add_message(
                    user_id, "assistant", response_text, scenario=scenario
                )
                ...
                if not last_use or last_use.date() < today:
                    await gamification_db.add_points(
                        user_id, 5, "assistant_daily_use", {"scenario": scenario}
                    )
        ...
        # Отправляем ответ
        await callback.message.edit_text(
            response_text, reply_markup=get_back_to_scenarios_keyboard(), parse_mode="HTML"
        )
        ...
        await callback.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        InlineKeyboardButton(
                            text="✅ Да, создать новый", callback_data="ai_confirm_new_plan"
                        ),
                        InlineKeyboardButton(
                            text="❌ Нет, оставить текущий", callback_data="assistant_menu"
                        ),
                    ]
                ]
            ),
...
async def process_chat_message(message: Message, state: FSMContext):
    ...
    if user_message.lower() in ["выход", "стоп", "отмена", "/stop"]:
        await state.clear()
        await message.answer("Режим чата завершен.", reply_markup=get_main_menu_keyboard())
        return
    ...
    try:
        ...
        if not openai_assistant.has_api_key():
            ...
            # Отправляем запрос к OpenAI
            response = await openai_assistant.send_message(message=user_message, context=history)
            ...
            if response["success"]:
                ...
                await assistant_db.add_message(user_id, "assistant", response_text)
            else:
                response_text = ASSISTANT_MESSAGES["error_api"]
        ...
        # Отправляем ответ
        await message.answer(
            response_text, reply_markup=get_chat_mode_keyboard(), parse_mode="HTML"
        )
...
async def show_chat_history(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.message.edit_text(
            text, reply_markup=get_assistant_menu_keyboard(), parse_mode="HTML"
        )
    except Exception as e:
    ...
    await callback.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="Да, очистить", callback_data="confirm_clear_history"
                    ),
