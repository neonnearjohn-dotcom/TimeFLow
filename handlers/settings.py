...
async def show_settings_menu(message: Message):
    ...
    try:
        ...
        await message.answer(
            "❌ Произошла ошибка при загрузке настроек", reply_markup=get_main_menu_keyboard()
        )
...
async def toggle_notifications_handler(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.message.edit_text(
            text, reply_markup=get_settings_keyboard(settings), parse_mode="HTML"
        )
...
async def set_theme_handler(callback: CallbackQuery):
    ...
    try:
        ...
        # Обновляем тему
        updated_settings = settings_db.update_settings(user_id, {"theme": theme})

        # Обновляем сообщение
        notifications_status = (
            "Включены" if updated_settings["notifications_enabled"] else "Выключены"
        )
        theme_names = {"system": "System", "light": "Light", "dark": "Dark"}
        ...
        await callback.message.edit_text(
            text, reply_markup=get_settings_keyboard(updated_settings), parse_mode="HTML"
        )
