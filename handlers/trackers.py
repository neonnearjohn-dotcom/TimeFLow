...
from keyboards.tracker import (
    get_tracker_menu_keyboard,
    get_habit_type_keyboard,
    get_habits_list_keyboard,
    get_habit_detail_keyboard,
    get_bad_habit_detail_keyboard,
    get_preset_habits_keyboard,
...
PRESET_HABITS = {
    ...
    "healthy_food": {
        ...
        "description": "Есть овощи и фрукты каждый день",
        "emoji": "▸",
    },
    ...
    "planning": {
        ...
        "description": "Планировать задачи на следующий день",
        "emoji": "▸",
    },
...
async def get_user_habits(self, user_id: int) -> List[Dict]:
    ...
    try:
        habits_ref = self.db.collection("users").document(str(user_id)).collection("habits")
...
async def get_user_bad_habits(self, user_id: int) -> List[Dict]:
    ...
    try:
        habits_ref = self.db.collection("users").document(str(user_id)).collection("bad_habits")
...
async def show_my_habits(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.message.edit_text(
            text, reply_markup=get_habits_list_keyboard(habits, "good"), parse_mode="HTML"
        )
    except Exception as e:
...
async def show_bad_habits_list(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.message.edit_text(
            text, reply_markup=get_habits_list_keyboard(habits, "bad"), parse_mode="HTML"
        )
    except Exception as e:
...
async def view_bad_habit_details(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.message.edit_text(
            text, reply_markup=get_bad_habit_detail_keyboard(habit_id), parse_mode="HTML"
        )
    except Exception as e:
...
async def confirm_reset_bad_habit(callback: CallbackQuery):
    ...
    builder.row(
        InlineKeyboardButton(text="Да, сбросить", callback_data=f"confirm_reset:{habit_id}"),
        InlineKeyboardButton(text="Отмена", callback_data=f"bad_habit_detail:{habit_id}"),
    )
...
async def show_habit_history(callback: CallbackQuery):
    ...
    try:
        ...
        builder = InlineKeyboardBuilder()
        builder.row(InlineKeyboardButton(text="◀ Назад", callback_data=f"habit_detail:{habit_id}"))
...
async def confirm_delete_habit(callback: CallbackQuery):
    ...
    builder.row(
        InlineKeyboardButton(
            text="Да, удалить", callback_data=f"confirm_delete:{habit_type}:{habit_id}"
        ),
        InlineKeyboardButton(
            text="Отмена",
...
async def delete_habit(callback: CallbackQuery):
    ...
    try:
        ...
        if success:
            await callback.message.edit_text(
                "Привычка удалена.", reply_markup=get_tracker_menu_keyboard(), parse_mode="HTML"
            )
...
async def show_tracker_stats(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.message.edit_text(
            text, reply_markup=get_tracker_menu_keyboard(), parse_mode="HTML"
        )
    except Exception as e:
...
async def cancel_creation(message: Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer("Создание привычки отменено.", reply_markup=get_main_menu_keyboard())
...
async def cancel_creation_callback(callback: CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    await callback.message.edit_text(
        "Создание привычки отменено.", reply_markup=get_tracker_menu_keyboard()
    )
    await callback.answer()
