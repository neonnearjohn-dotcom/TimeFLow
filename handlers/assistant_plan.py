...
async def cmd_plan(message: Message):
    ...
    try:
        ...
        await message.answer(
            "<b>❌ Ошибка</b>\n\n" "Не удалось загрузить план. Попробуйте позже.", parse_mode="HTML"
        )
...
async def generate_plan_handler(message: Message, telegram_id: int, state: FSMContext):
    ...
    try:
        ...
        status_msg = await message.answer(
            "<b>⏳ Генерирую персональный план...</b>", parse_mode="HTML"
        )
                ...
                reply_markup=InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            ...
                            InlineKeyboardButton(
                                text="❌ Нет, оставить текущий", callback_data="plan:view"
                            ),
                        ]
                    ]
                ),
...
async def show_plan_preview(message: Message, plan, start_day: int, is_view_mode: bool = False):
    ...
    checkpoints_in_range = [cp for cp in plan.checkpoints if start_day <= cp.day_number <= end_day]
    ...
    if is_view_mode:
        ...
        if start_day > 1:
            nav_row.append(
                InlineKeyboardButton(text="◀️ Назад", callback_data=f"plan:prev:{start_day}")
            )
    ...
    await callback.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="✅ Да, создать новый", callback_data="plan:regenerate_confirm"
                    ),
                    InlineKeyboardButton(text="❌ Отмена", callback_data="ai_plan_menu"),
                ]
            ]
        ),
        parse_mode="HTML",
