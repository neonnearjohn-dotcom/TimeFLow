...
class FocusScheduler:
    ...
    async def start_timer(
        ...
        try:
            ...
            timer_info.task = asyncio.create_task(self._run_timer(timer_info, on_complete, on_tick))
    ...
    async def resume_timer(
        ...
        try:
            ...
            logger.info(f"Таймер {session_id} возобновлен с {remaining_minutes} минут")
            return True
    ...
    def get_active_timers_count(self) -> int:
        ...
        return len([t for t in self.timers.values() if t.state == TimerState.RUNNING])
    ...
    async def schedule_task(self, delay_seconds: int, callback: Callable[[], Any]) -> asyncio.Task:
        """
    ...
    async def _run_timer(
        self, timer: TimerInfo, on_complete: Callable, on_tick: Optional[Callable]
    ):
        ...
        try:
            ...
            while timer.elapsed_minutes < timer.total_minutes:
                ...
                if on_tick and (now - last_tick_time).total_seconds() >= 60:
                    ...
                    if asyncio.iscoroutinefunction(on_tick):
                        await on_tick(timer.session_id, timer.user_id, remaining)
                    else:
                        on_tick(timer.session_id, timer.user_id, remaining)
            ...
            if timer.state == TimerState.RUNNING:
                ...
                if asyncio.iscoroutinefunction(on_complete):
                    await on_complete(timer.session_id, timer.user_id, timer.total_minutes)
                else:
                    on_complete(timer.session_id, timer.user_id, timer.total_minutes)
    ...
    async def _monitor_timers(self):
        ...
        try:
            while self._running:
                ...
                if active_count > 0:
                    logger.debug(
                        f"Активных таймеров: {active_count}, " f"всего: {len(self.timers)}"
                    )
