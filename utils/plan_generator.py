...
from models.ai_profile import PlanData as Plan, DayTask, Checkpoint, BufferDay, TaskStatus
...
def calculate_horizon_days(
    answers: dict, constraints: dict, category: str, default_max: int = 30
) -> int:
    ...
    category_horizons = {
        "exam": {"новичок": 21, "начинающий": 21, "средний": 14, "продвинутый": 10, "эксперт": 7},
        "skill": {"новичок": 30, "начинающий": 30, "средний": 21, "продвинутый": 14, "эксперт": 10},
        "habit": {"новичок": 21, "начинающий": 21, "средний": 21, "продвинутый": 14, "эксперт": 14},
        "health": {
            "новичок": 14,
            "начинающий": 14,
            "средний": 21,
        ...
        "time": {"новичок": 7, "начинающий": 7, "средний": 14, "продвинутый": 21, "эксперт": 30},
    }
...
def validate_plan_constraints(
    plan_json: dict, constraints: dict, sessions_per_day: Optional[int] = None
) -> Tuple[bool, List[str]]:
    ...
    for day_data in plan_json.get("days", []):
            ...
            if time_str:
                try:
                    ...
                    if task_time_minutes >= limit_time_minutes:
                        errors.append(
                            f"День {day_num}: задача в {time_str} после ограничения {no_study_after}"
                        )
                        ...
                        if "-" in window:
                            try:
                                ...
                                if start_minutes <= task_time_minutes <= end_minutes:
                                    errors.append(
                                        f"День {day_num}: задача в {time_str} попадает в blackout {window}"
                                    )
                            except:
                                pass
                except:
        ...
        if total_minutes > daily_minutes:
            errors.append(
                f"День {day_num}: суммарное время {total_minutes} мин > лимита {daily_minutes} мин"
            )
...
def validate_plan_json(plan_json: Any) -> Optional[str]:
    ...
    for i, day in enumerate(plan_json["days"]):
        ...
        for j, task in enumerate(day["tasks"]):
            ...
            if len(activity) > 300:
                logger.warning(
                    f"Слишком длинное название активности ({len(activity)} символов) в задаче {j+1} дня {i+1}"
                )
...
def parse_or_fix_json(text: str) -> dict:
    ...
    if re.search(r"([{,]\s*)([A-Za-z_][A-Za-z0-9_]*)\s*:", text) and not re.search(
        r"([{,]\s*)([A-Za-z_][A-Za-z0-9_]*)\s*:", fixed_text
    ):
    ...
    if open_braces != close_braces or open_brackets != close_brackets:
        logger.info(
            f"Дисбаланс скобок: {{:{open_braces} }}:{close_braces} [:{open_brackets} ]:{close_brackets}"
        )
...
async def generate_plan(
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            ...
            logger.info(
                f"Запрашиваем план у GPT для категории {category} через generate_json_response"
            )
            ...
            if plan_json is None:
                logger.warning(
                    "generate_json_response не смог распарсить JSON, пробуем восстановить"
                )
                ...
                if attempt == max_attempts:
                    ...
                    return await generate_plan_deterministic(
                        category, answers, constraints, horizon_days
                    )
            ...
            is_valid, validation_errors = validate_plan_constraints(
                plan_json, constraints, sessions_per_day
            )
            ...
            if len(plan_json["days"]) > days_to_generate:
                logger.warning(
                    f"GPT сгенерировал {len(plan_json['days'])} дней вместо {days_to_generate}, обрезаем"
                )
                    ...
                    plan_json["days"].append(
                        {
                            ...
                            "tasks": [
                                ...
                                {
                                    "time": "14:00",
                                    "activity": f"Дополнительная задача дня {day_num}",
                                },
                            ],
                        }
                    )
            ...
            logger.debug(
                f"Сгенерировано дней: {len(plan_json.get('days', []))}, задач: {sum(len(d.get('tasks', [])) for d in plan_json.get('days', []))}"
            )
            return plan
...
async def load_plan_prompt() -> Dict[str, Any]:
    ...
    try:
        prompt_path = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "ai_prompts", "plan_prompts.json"
        )
...
async def generate_plan_deterministic(
    category: str, answers: dict, constraints: dict, horizon_days: int = 15
) -> Plan:
    """
    ...
    for day_num in range(1, horizon_days + 1):
        ...
        is_blackout = any(day_name in period for period in blackout)
...
def _generate_day_slots(
    ...
    if category == "exam":
        slots = _generate_exam_slots(
            day_num, num_slots, minutes_per_slot, current_hour, current_minute, hour_limit
        )
    elif category == "skill":
...
def _generate_exam_slots(
    ...
    for i in range(num_slots):
        ...
        slots.append(PlanSlot(time=time_str, task=template, est_min=minutes_per_slot))
...
def _generate_habit_slots(
    ...
    slots.append(PlanSlot(time="07:00-07:30", task="Утренний ритуал привычки", est_min=30))

    # Анти-триггер слот (середина дня)
    if num_slots >= 2:
        slots.append(
            PlanSlot(time="14:00-14:20", task="Работа с триггерами и соблазнами", est_min=20)
        )

    # Вечерний якорь
    if num_slots >= 3:
        slots.append(PlanSlot(time="20:00-20:30", task="Вечернее закрепление привычки", est_min=30))

    # План B (короткая альтернатива)
    if num_slots >= 4:
        slots.append(
            PlanSlot(time="12:00-12:10", task="Экспресс-версия привычки (план B)", est_min=10)
        )
...
def _generate_health_slots(
    ...
    if intensity == "отдых":
        ...
        slots.append(
            PlanSlot(
                time="09:00-09:30", task="Легкая растяжка и дыхательные упражнения", est_min=30
            )
        )
        if num_slots >= 2:
            slots.append(
                PlanSlot(time="18:00-18:30", task="Вечерняя йога или медитация", est_min=30)
            )
    else:
        # Тренировочные дни
        templates = {
        ...
        for i in range(min(num_slots, len(day_templates))):
            ...
            slots.append(
                PlanSlot(time=time_str, task=f"{template} ({intensity} уровень)", est_min=duration)
            )
...
def _generate_time_slots(
    ...
    productivity_slots = [
        PlanSlot(time="09:00-10:30", task="Глубокая работа (пик продуктивности)", est_min=90),
        PlanSlot(time="11:00-12:00", task="Важные встречи и коммуникации", est_min=60),
        PlanSlot(time="14:00-15:30", task="Фокусная работа (без отвлечений)", est_min=90),
        PlanSlot(time="16:00-17:00", task="Планирование и подведение итогов", est_min=60),
    ]
    ...
    async def test_plan_generator():
        ...
        for category in categories:
            ...
            for day_num in range(1, 8):
                ...
                print(
                    f"День {day_num}: {total_minutes} минут (лимит: {test_constraints['daily_minutes']})"
                )
        ...
        for test in test_cases:
            ...
            try:
                ...
                if isinstance(result, dict) and "days" in result:
                    print(
                        f"✅ УСПЕХ: JSON успешно {'восстановлен' if test['should_fix'] else 'распарсен'}"
                    )
