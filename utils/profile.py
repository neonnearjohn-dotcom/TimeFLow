...
from keyboards.profile import (
    ...
    get_stats_keyboard,
    get_achievement_categories_keyboard,
...
async def show_user_profile(user_id: int, answer_method):
    ...
    try:
        ...
        if not profile:
            await answer_method(
                "😔 Профиль не найден. Попробуй выполнить хотя бы одно действие!",
        ...
        if created_at:
            days_with_us = (datetime.utcnow() - created_at).days
        ...
        await answer_method(text, reply_markup=get_profile_menu_keyboard(), parse_mode="HTML")

    except Exception as e:
        logger.error(f"Ошибка при показе профиля: {e}")
        await answer_method(ERROR_MESSAGES["unknown_error"])
...
async def show_achievements_menu(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.message.edit_text(
            text, reply_markup=get_achievements_keyboard(bool(achievements)), parse_mode="HTML"
        )
    except Exception as e:
...
async def show_achievements_by_category(callback: CallbackQuery):
    ...
    try:
        ...
        user_ach_ids = [
            ach.get("achievement_id") for ach in user_achievements if "achievement_id" in ach
        ]
        ...
        await callback.message.edit_text(
            text, reply_markup=get_achievement_categories_keyboard(), parse_mode="HTML"
        )
    except Exception as e:
...
async def show_achievements_progress(callback: CallbackQuery):
    ...
    try:
        achievements = await gamification_db.get_user_achievements(user_id)
        user_ach_ids = [
            ach.get("achievement_id") for ach in achievements if "achievement_id" in ach
        ]
        ...
        await callback.message.edit_text(
            text, reply_markup=get_achievements_keyboard(), parse_mode="HTML"
        )
    except Exception as e:
...
async def show_points_history(callback: CallbackQuery):
    ...
    try:
        ...
        await callback.message.edit_text(
            text, reply_markup=get_back_to_profile_keyboard(), parse_mode="HTML"
        )
    except Exception as e:
