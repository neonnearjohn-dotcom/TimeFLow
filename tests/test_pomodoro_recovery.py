...
async def test_recovery_marks_expired_sessions_once(pomodoro_service, mock_notify):
    ...
    with patch.object(
        ...
        with patch.object(pomodoro_service, "mark_done_and_notify", return_value=True) as mock_mark:
            # Запускаем recovery
            ...
            mock_mark.assert_called_once_with(session_id=session_id, notify=mock_notify)
...
async def test_scheduler_idempotent_under_race(pomodoro_service, mock_notify):
    ...
    with patch.object(pomodoro_service, "mark_done_and_notify", side_effect=side_effect):
...
async def test_no_action_for_future_session(pomodoro_service, mock_notify):
    ...
    with patch("asyncio.to_thread", side_effect=lambda f, *args: f(*args)):
        result = await pomodoro_service.mark_done_and_notify(session_id, mock_notify)
...
async def test_handles_empty_set_gracefully(pomodoro_service, mock_notify):
    ...
    with patch.object(pomodoro_service, "fetch_expired_active", return_value=[]) as mock_fetch:
        # Запускаем recovery
        processed = await recovery_pass(pomodoro_service, mock_notify)
