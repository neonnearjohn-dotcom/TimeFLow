...
async def add_test_points(message: Message):
    ...
    try:
        ...
        success, balance = await gamification_db.add_points(
            message.from_user.id, 100, "test_points", {"test": True}
        )

        if success:
            await message.answer(f"✅ Добавлено 100 очков!\n" f"Новый баланс: {balance}")
        else:
            await message.answer("❌ Не удалось добавить очки")

    except Exception as e:
        await message.answer(f"❌ Ошибка при добавлении очков:\n{str(e)}")
...
async def test_achievement(message: Message):
    ...
    try:
        ...
        success, points = await gamification_db.unlock_achievement(
            message.from_user.id, "first_habit"
        )
        ...
        await message.answer(f"❌ Ошибка при получении достижения:\n{str(e)}")
