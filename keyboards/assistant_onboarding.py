...
def get_onboarding_start_keyboard() -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(text="🚀 Начать настройку", callback_data="ai_assistant_start")
    )
    builder.row(InlineKeyboardButton(text="ℹ️ Как это работает?", callback_data="ai_how_it_works"))
...
def get_plan_preview_keyboard(start_day: int, horizon_days: int) -> InlineKeyboardMarkup:
    ...
    if start_day > 1:
        nav_row.append(InlineKeyboardButton(text="◀️ Назад", callback_data=f"plan:prev:{start_day}"))

    # Кнопка "Дальше" (только если есть еще дни)
    if start_day + 2 < horizon_days:  # +2 потому что показываем 3 дня
        nav_row.append(
            InlineKeyboardButton(text="Дальше ▶️", callback_data=f"plan:next:{start_day}")
        )
    ...
    action_row = [
        ...
        InlineKeyboardButton(text="✖️ Отмена", callback_data="plan:cancel"),
    ]
...
def get_plan_saved_keyboard() -> InlineKeyboardMarkup:
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="📊 Мой прогресс", callback_data="ai_progress")],
...
def get_plan_generate_keyboard() -> InlineKeyboardMarkup:
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="📅 Сгенерировать план", callback_data="plan:generate")],
...
def get_plan_management_keyboard() -> InlineKeyboardMarkup:
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="📄 Просмотреть план", callback_data="plan:open_preview")],
            [InlineKeyboardButton(text="🔄 Пересоздать план", callback_data="plan:regenerate")],
...
def get_category_selection_keyboard(categories: Dict[str, Dict]) -> InlineKeyboardMarkup:
    ...
    builder.row(InlineKeyboardButton(text="❓ Помощь в выборе", callback_data="onb_help_choose"))
    builder.row(InlineKeyboardButton(text="◀️ Назад", callback_data="assistant_menu"))
...
def get_progress_keyboard(current: int, total: int) -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(text=f"[{progress_bar}] {current}/{total}", callback_data="progress")
    )
...
def get_skip_or_back_keyboard(can_skip: bool = True) -> InlineKeyboardMarkup:
    ...
    buttons = []
    if can_skip:
        buttons.append(InlineKeyboardButton(text="Пропустить ➡️", callback_data="onb_skip_current"))
...
def get_time_selection_keyboard() -> InlineKeyboardMarkup:
    ...
    builder.row(
        *[InlineKeyboardButton(text=time, callback_data=f"onb_time_{time}") for time in day_times]
    )
    ...
    builder.row(InlineKeyboardButton(text="✏️ Ввести вручную", callback_data="onb_time_manual"))
...
def get_date_quick_select_keyboard() -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(text="📅 Выбрать другую дату", callback_data="onb_date_manual")
    )
...
def get_number_quick_select_keyboard(
    min_val: int, max_val: int, step: int = 1, unit: str = ""
    ...
    builder.row(InlineKeyboardButton(text="✏️ Ввести вручную", callback_data="onb_number_manual"))
...
def get_help_keyboard() -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(text="📚 Экзамены", callback_data="onb_help_exam"),
        InlineKeyboardButton(text="🎯 Навыки", callback_data="onb_help_skill"),
    )
    builder.row(
        InlineKeyboardButton(text="🌱 Привычки", callback_data="onb_help_habit"),
        InlineKeyboardButton(text="💪 Здоровье", callback_data="onb_help_health"),
    )
    builder.row(InlineKeyboardButton(text="⏰ Время", callback_data="onb_help_time"))
    builder.row(InlineKeyboardButton(text="◀️ Назад к выбору", callback_data="ai_assistant_start"))
...
def get_example_keyboard(category: str) -> InlineKeyboardMarkup:
    ...
    if category in examples:
        for label, value in examples[category]:
            builder.row(
                InlineKeyboardButton(
                    text=f"→ {label}", callback_data=f"onb_example_{category}_{value}"
                )
            )

    builder.row(InlineKeyboardButton(text="✏️ Свой вариант", callback_data=f"onb_custom_{category}"))
    builder.row(InlineKeyboardButton(text="◀️ Назад", callback_data="ai_assistant_start"))
...
def get_error_keyboard() -> InlineKeyboardMarkup:
    ...
    builder.row(InlineKeyboardButton(text="🔄 Попробовать снова", callback_data="onb_retry"))
    builder.row(InlineKeyboardButton(text="💬 Связаться с поддержкой", callback_data="support"))
    builder.row(InlineKeyboardButton(text="◀️ В меню", callback_data="assistant_menu"))
