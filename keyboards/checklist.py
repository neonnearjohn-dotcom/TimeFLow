...
def get_priority_keyboard() -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(
            text="● Важно и срочно",
            callback_data="priority:urgent_important"
        )
    )
    builder.row(
        InlineKeyboardButton(
            text="● Важно, но не срочно",
            callback_data="priority:not_urgent_important"
        )
    )
    builder.row(
        InlineKeyboardButton(
            text="● Срочно, но не важно",
            callback_data="priority:urgent_not_important"
        )
    )
    builder.row(
        InlineKeyboardButton(
            text="● Не важно и не срочно",
            callback_data="priority:not_urgent_not_important"
        )
    )
    builder.row(
...
def get_tasks_list_keyboard(tasks: List[Dict[str, Any]], task_type: str = "all", 
                           page: int = 1, tasks_per_page: int = 10) -> InlineKeyboardMarkup:
        ...
        if total_pages > 1:
            ...
            if page > 1:
                nav_buttons.append(
                    InlineKeyboardButton(
                        text="◀ Пред.",
                        callback_data=f"tasks_page:{task_type}:{page-1}"
                    )
                )
...
def get_task_actions_keyboard(task_id: str, is_completed: bool) -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(text="◀ К задачам", callback_data="all_tasks")
    )
...
def get_deadline_keyboard() -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(text="Через месяц", callback_data="deadline:month")
    )
    builder.row(
...
def get_edit_field_keyboard(task_id: str) -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(text="◀ Отмена", callback_data=f"task_detail:{task_id}")
    )
