...
def get_habit_detail_keyboard(habit_id: str, is_completed_today: bool = False) -> InlineKeyboardMarkup:
    """
...
def get_bad_habit_detail_keyboard(habit_id: str) -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(text="Срыв", callback_data=f"reset_bad_habit:{habit_id}")
    )
    builder.row(
    ...
    builder.row(
        InlineKeyboardButton(text="◀ Назад", callback_data="bad_habits")
    )
...
def get_habit_type_keyboard() -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(text="✅ Полезная привычка", callback_data="habit_type:good")
    )
    builder.row(
        InlineKeyboardButton(text="🚫 Вредная привычка", callback_data="habit_type:bad")
    )
    builder.row(
        InlineKeyboardButton(text="📋 Выбрать из пресетов", callback_data="habit_type:preset")
    )
    builder.row(
        InlineKeyboardButton(text="❌ Отмена", callback_data="cancel_habit_creation")
    )
...
def get_habits_list_keyboard(habits: List[Dict], habit_type: str = "good") -> InlineKeyboardMarkup:
    ...
    for habit in habits:
        ...
        callback_data = f"habit_detail:{habit_id}" if habit_type == "good" else f"bad_habit_detail:{habit_id}"
    ...
    # Кнопка назад
    builder.row(
        InlineKeyboardButton(text="◀ Назад", callback_data="tracker_menu")
    )
    ...
    if habit_type == "good":
        ...
        builder.row(InlineKeyboardButton(text="➕ Добавить вредную привычку", callback_data="add_bad_habit"))
...
def get_habit_actions_keyboard(habit_id: str, completed_today: bool = False) -> InlineKeyboardMarkup:
    """
    ...
    if not completed_today:
        builder.row(
            InlineKeyboardButton(text="✅ Выполнено сегодня", callback_data=f"complete_habit:{habit_id}")
        )
    else:
        builder.row(
    ...
    builder.row(
        InlineKeyboardButton(text="◀️ Назад", callback_data="my_habits")
    )
...
def get_bad_habit_actions_keyboard(habit_id: str) -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(text="◀️ Назад", callback_data="bad_habits")
    )
