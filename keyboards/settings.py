...
def get_settings_keyboard(settings: Dict[str, Any]) -> InlineKeyboardMarkup:
    ...
    notifications_text = "🔔 Уведомления: " + (
        "Вкл ✅" if settings["notifications_enabled"] else "Выкл ❌"
    )
    builder.row(InlineKeyboardButton(text=notifications_text, callback_data="toggle_notifications"))

    # Кнопки выбора темы
    theme_buttons = []
    themes = [("system", "📱 System"), ("light", "☀️ Light"), ("dark", "🌙 Dark")]
    ...
    for theme_value, theme_text in themes:
        ...
        theme_buttons.append(
            InlineKeyboardButton(text=button_text, callback_data=f"set_theme:{theme_value}")
        )

    # Добавляем заголовок для темы
    builder.row(
        InlineKeyboardButton(
            text="🎨 Тема оформления:", callback_data="theme_header"  # Неактивная кнопка-заголовок
        )
    )
    ...
    # Кнопка возврата
    builder.row(InlineKeyboardButton(text="🔙 Назад в меню", callback_data="settings_back_to_main"))
