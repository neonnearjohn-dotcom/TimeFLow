...
class ExamConstraints(BaseModel):
    ...
    preferred_study_time: Optional[str] = Field(None, description="Предпочтительное время (утро/день/вечер)")
...
class SkillConstraints(BaseModel):
    ...
    equipment_required: List[str] = Field(default_factory=list, description="Необходимое оборудование")
    location_constraints: Optional[str] = Field(None, description="Ограничения по месту")
    mentor_availability: Optional[Dict[int, List[str]]] = Field(None, description="Доступность ментора")
...
class HabitConstraints(BaseModel):
    ...
    environmental_changes: List[str] = Field(default_factory=list, description="Изменения окружения")


class HealthConstraints(BaseModel):
    """Ограничения для здоровья и фитнеса"""
    medical_conditions: List[str] = Field(default_factory=list, description="Медицинские ограничения")
    injury_history: List[str] = Field(default_factory=list, description="История травм")
    available_time_slots: Dict[int, List[str]] = Field(default_factory=dict, description="Доступное время")
    gym_access: bool = Field(False, description="Доступ в спортзал")
    dietary_restrictions: List[str] = Field(default_factory=list, description="Диетические ограничения")
...
class TimeConstraints(BaseModel):
    ...
    work_hours: Dict[int, Dict[str, str]] = Field(default_factory=dict, description="Рабочие часы по дням")
    fixed_commitments: List[Dict[str, any]] = Field(default_factory=list, description="Фиксированные обязательства")
    ...
    energy_levels: Dict[str, int] = Field(default_factory=dict, description="Уровни энергии по времени дня")
    interruption_windows: List[Dict[str, str]] = Field(default_factory=list, description="Окна прерываний")
...
class TimeSlot(BaseModel):
    """Временной слот"""
    start: str = Field(..., pattern="^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$", description="Начало (HH:MM)")
