...
def create_exam_profile_example() -> AIProfile:
    ...
    onboarding = OnboardingData(
        completed=True, answers=exam_answers.dict(), completed_at=datetime.now()
    )
    ...
    profile = AIProfile(
        ...
        preferences=PreferencesData(
            theme=ThemeType.LIGHT, notifications_enabled=True, reminder_time="19:00", language="ru"
        ),
    )
...
def create_habit_profile_example() -> AIProfile:
    ...
    onboarding = OnboardingData(completed=True, answers=habit_answers.dict())
    ...
    plan = PlanData(type=CategoryType.HABIT, horizon_days=21, days=days, checkpoints=checkpoints)
    ...
    constraints = ConstraintsData(daily_time_minutes=10, habit_constraints=habit_constraints.dict())
