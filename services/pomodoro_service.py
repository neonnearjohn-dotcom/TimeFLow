...
from utils.logging import get_logger, get_request_id, get_user_id
...
class PomodoroService:
    ...
    async def create_session(
        ...
        # Асинхронная обёртка для синхронного Firestore
    ...
    async def fetch_expired_active(self, limit: int = 100) -> List[tuple[str, PomodoroSession]]:
        ...
        # Асинхронный запрос к Firestore
    ...
    async def mark_done_and_notify(
        ...
        """
        Помечает сессию как завершённую и отправляет уведомление.
        Идемпотентная операция через транзакцию.
        """
        
        @firestore.transactional
        def update_in_transaction(transaction: Transaction) -> tuple[bool, Optional[str]]:
            """Транзакционное обновление сессии."""
            ...
            # Если ещё не уведомляли
            if not data.get("last_notified_at"):
