...
class FocusService:
    ...
    async def resume_session(self, user_id: str) -> Dict[str, Any]:
        ...
        await self.db.update_session(
            session["id"],
            {"status": SessionStatus.ACTIVE.value, "resumed_at": datetime.now(timezone.utc)},
        )
    ...
    async def stop_session(self, user_id: str, completed: bool = False) -> Tuple[int, bool]:
        """
        ...
        if (
            is_completed
            and session["type"] == SessionType.WORK.value
            and session.get("auto_start_break", True)
        ):
            try:
                ...
                # Запускаем перерыв
                await self.start_session(user_id, session_type=break_type, auto_start_break=False)
    ...
    async def get_session_info(self, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        if session["status"] == SessionStatus.ACTIVE.value:
            ...
            session["remaining_minutes"] = session["duration_minutes"] - session.get(
                "completed_minutes", 0
            )
    ...
    async def get_statistics(self, user_id: str, period: str = "today") -> Dict[str, Any]:
        """
    ...
    async def restore_active_sessions(self) -> int:
        ...
        for session in sessions:
            try:
                ...
                logger.error(f"Ошибка восстановления сессии {session['id']}: {e}", exc_info=True)
    ...
    async def _on_session_complete(self, session_id: str, user_id: str, completed_minutes: int):
    ...
    async def _on_session_tick(self, session_id: str, user_id: str, remaining_minutes: int):
        """Callback при тике таймера (каждую минуту)"""
        logger.debug(f"Тик сессии {session_id}: осталось {remaining_minutes} минут")
    ...
    async def _update_user_stats(self, user_id: str, completed_minutes: int):
        """Обновляет статистику пользователя"""
        await self.db.increment_stats(user_id, completed_minutes)
        logger.info(f"Обновлена статистика пользователя {user_id}: " f"+{completed_minutes} минут")
