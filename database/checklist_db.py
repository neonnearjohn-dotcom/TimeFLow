...
class ChecklistDB:
    ...
    async def get_user_tasks(
        self, telegram_id: int, priority: Optional[str] = None, status: str = "active"
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def get_tasks_by_priority(
        self, telegram_id: int, priority: str, status: str = "active"
    ) -> List[Dict[str, Any]]:
    ...
    async def complete_task(self, telegram_id: int, task_id: str) -> Tuple[bool, int]:
        ...
        try:
            ...
            # Обновляем задачу
            task_ref.update({"status": "completed", "completed_at": datetime.utcnow()})
    ...
    async def get_completed_tasks_history(
        ...
        try:
            ...
            completed = (
                user_ref.collection("completed_tasks")
