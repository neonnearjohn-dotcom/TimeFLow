...
# Импорт моделей
from models.ai_profile import AIProfile, CategoryType, DayTask, Checkpoint, TaskStatus, ThemeType
...
async def example_onboarding_flow(profile_db: AssistantProfileDB, telegram_id: int):
    ...
    success = await profile_db.finalize_onboarding(telegram_id, "exam", answers, constraints)
        ...
        "checkpoints": [
            {
                ...
                "criteria": ["Решено 50+ задач", "Пройдены базовые темы", "Средний балл > 70%"],
                "status": "pending",
            }
        ],
        "buffer_days": [
...
async def example_progress_update(profile_db: AssistantProfileDB, telegram_id: int):
    ...
    await profile_db.update_progress(
        telegram_id, {"days_done": 3, "completion_rate": 0.1, "streak_current": 3}
    )
    print("✓ Обновлены базовые поля прогресса")

    # 2. Инкрементальное обновление
    await profile_db.update_progress(telegram_id, {"increment_days_done": 1, "increment_streak": 1})
    print("✓ Инкрементально обновлены счетчики")

    # 3. Добавление причины неудачи
    await profile_db.update_progress(
        telegram_id, {"add_fail_reason": "Не хватило времени на выполнение"}
    )
    ...
    await profile_db.update_progress(
        telegram_id,
        {"completion_rate": 0.15, "streak_best": 4, "last_checkin": datetime.now(timezone.utc)},
    )
...
async def example_task_management(profile_db: AssistantProfileDB, telegram_id: int):
    ...
    if profile.plan.days:
        ...
        success = await profile_db.update_task_status(
            telegram_id, task_id, "completed", "Выполнено успешно, все тесты решены"
        )
